...
def calculate_diaphragm_cross_sectional_area(image_dir_path, image_filename):
    image = cv2.imread(image_dir_path + '/' + image_filename)
    original = image.copy()

    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    # lower = np.array([0, 0, 114], dtype="uint8")
    # upper = np.array([4, 7, 255], dtype="uint8")
    lower = np.array([0, 0, 174], dtype="uint8")
    upper = np.array([4, 7, 179], dtype="uint8")
