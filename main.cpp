...
int main(int argc, char *argv[]) {
    Point_set points;
    std::string fname =
            argc == 1 ? CGAL::data_file_path("lung/lung_base_point_cloud.ply") : argv[1];
    if (argc < 2) {
    ...
    int y_max = std::ceil((*y_max_point)[0]);
    ...
    int z_max = std::ceil((*z_max_point)[0]);
    std::cout << z_max << std::endl;


//    std::cout << (points.points().begin()) << std::endl;
...
//    // f << lung_base_mesh;
//    // f.close();
//    CGAL::IO::write_polygon_mesh("/usr/local/lib/cgal/data/lung/1.ply", lung_base_mesh);
