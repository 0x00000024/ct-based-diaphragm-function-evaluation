...
class HounsfieldUnitThresholder:
    ...
    def __init__(self, lower_bound: int, upper_bound: int, z_value,
        ...
        self._pixel_data: NDArray[Shape[rows, columns], Int16] = self._metadata.pixel_data
        self._denoised_data: NDArray[Shape[rows, columns], Int8] = self._metadata.denoised_data
    ...
    def extract_substance_3d_coordinates(self) -> List[List[float]]:
        ...
        hu_data: NDArray[Shape[rows, columns], Int16] = np.zeros(shape=(rows, columns))
        ...
        for i in range(0, rows, step):
            for j in range(0, columns, step):
                ...
                if self._denoised_data[i][j] == 0:
                    continue
                hu_data[i][j] = self._pixel_data[i][j] * self._rescale_slope + self._rescale_intercept
